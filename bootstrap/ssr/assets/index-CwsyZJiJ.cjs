...
function createCollection(name) {
  ...
  const [createCollectionContext, createCollectionScope] = ssr.createContextScope(PROVIDER_NAME);
  ...
  const CollectionProvider = (props) => {
    ...
    const ref = ssr.React2.useRef(null);
    const itemMap = ssr.React2.useRef(/* @__PURE__ */ new Map()).current;
    return /* @__PURE__ */ ssr.jsxRuntimeExports.jsx(CollectionProviderImpl, { scope, itemMap, collectionRef: ref, children });
  };
  CollectionProvider.displayName = PROVIDER_NAME;
  const COLLECTION_SLOT_NAME = name + "CollectionSlot";
  const CollectionSlotImpl = /* @__PURE__ */ createSlot(COLLECTION_SLOT_NAME);
  const CollectionSlot = ssr.React2.forwardRef(
    (props, forwardedRef) => {
      const { scope, children } = props;
      const context = useCollectionContext(COLLECTION_SLOT_NAME, scope);
      const composedRefs = ssr.useComposedRefs(forwardedRef, context.collectionRef);
      return /* @__PURE__ */ ssr.jsxRuntimeExports.jsx(CollectionSlotImpl, { ref: composedRefs, children });
    }
  );
  ...
  const CollectionItemSlot = ssr.React2.forwardRef(
    (props, forwardedRef) => {
      const { scope, children, ...itemData } = props;
      const ref = ssr.React2.useRef(null);
      const composedRefs = ssr.useComposedRefs(forwardedRef, ref);
      const context = useCollectionContext(ITEM_SLOT_NAME, scope);
      ssr.React2.useEffect(() => {
        context.itemMap.set(ref, { ref, ...itemData });
        return () => void context.itemMap.delete(ref);
      });
      return /* @__PURE__ */ ssr.jsxRuntimeExports.jsx(CollectionItemSlotImpl, { ...{ [ITEM_DATA_ATTR]: "" }, ref: composedRefs, children });
    }
  );
  CollectionItemSlot.displayName = ITEM_SLOT_NAME;
  function useCollection(scope) {
    const context = useCollectionContext(name + "CollectionConsumer", scope);
    const getItems = ssr.React2.useCallback(() => {
